function createCharacterHead(api, geometry, appearance) {
  ...
  const profile = faceStyles[appearance.face];
  loft("Face", "head", [[47.2, 0, 0], [48, profile.jaw ? 5.4 : 3.8, 3.2], [50, profile.jaw ? 9.6 : 8.6, 5.8], [54, 13.2, 8.4], [59, 14.5, 9.4], [65, 14.1, 9.6], [71, 11.5, 8.2], [75, 4.5, 3.6], [76, 0, 0]], "skin");
  for (const side of [-1, 1]) {
    ...
    const eyeHeight = profile.height;
    const eye = (name, points, material, depth) => patch(`${prefix} ${name}`, "eyes", points.map(([u, v]) => {
      const x = side * (6 + u);
      return [x, 58.6 + v * eyeHeight + u * profile.tilt, 10.15 - Math.abs(x) * 0.13 + depth];
    }), material);
    if (profile.wink && side === 1) {
    ...
    const browTilt = profile.brow;
    const browTop = profile.heavyBrow ? 65.6 : 64.9;
    patch(`${prefix} eyebrow`, "head", [[side * 3.8, 64, 9.7], [side * 7.9, 64 + browTilt, 9.2], [side * 8.3, browTop + browTilt, 9.1], [side * 4.3, browTop, 9.7]], "hairDark");
    ellipsoid(`${prefix} cheek`, "head", [side * 9.1, 54.5, 7.5], [profile.blush ?? 1.6, 0.65, 0.3], "blush", false);
  }
  patch("Nose light", "head", [[-0.55, 54.5, 9.25], [0.45, 54.5, 9.25], [0.2, 55.4, 9.4]], "skinLight");
  const smiling = profile.smile;
  ...
  createFaceDetails(geometry, appearance, profile);
