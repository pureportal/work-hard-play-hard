...
import {
  ...
  CHARACTER_FOOT_ANCHOR, CHARACTER_SEAT_ANCHOR, CHARACTER_FACES, CHARACTER_GENDERS, CHARACTER_HAIRSTYLES,
  CHARACTER_OUTFITS, CHARACTER_HEADWEAR, DEFAULT_CHARACTER_APPEARANCE,
} from "../../../packages/shared/src/character.ts";

const workspace = fileURLToPath(new URL("../../../", import.meta.url));
const source = await Promise.all(["geometry.cjs", "head.cjs", "hair.cjs", "clothing.cjs", "wardrobe.cjs", "headwear.cjs", "customization.cjs", "model.cjs", "animations.cjs", "render.cjs"].map(async name =>
...
for (const gender of CHARACTER_GENDERS) {
  for (const face of CHARACTER_FACES) designs.push({ layer: "head", name: `${gender}-${face}`, appearance: { ...DEFAULT_CHARACTER_APPEARANCE, gender, face } });
  for (const outfit of CHARACTER_OUTFITS) {
    designs.push({ layer: "upper", name: `${gender}-${outfit}-flat`, appearance: { ...DEFAULT_CHARACTER_APPEARANCE, gender, upperBody: outfit } });
    designs.push({ layer: "lower", name: `${gender}-${outfit}`, appearance: { ...DEFAULT_CHARACTER_APPEARANCE, gender, lowerBody: outfit } });
    designs.push({ layer: "shoes", name: `${gender}-${outfit}`, appearance: { ...DEFAULT_CHARACTER_APPEARANCE, gender, shoes: outfit } });
  }
}
