import {
  ...
  approvalCaseCoins, approvalCaseDurationMs, approvalCaseOutput, approvalDeskLevel,
...
export class ApprovalDeskStore {
  ...
  start(userId: string, caseId: ApprovalCaseId, now = new Date()): void {
    if (!APPROVAL_CASES.some((item) => item.id === caseId)) throw new Error("APPROVAL_CASE_INVALID");
    const player = this.player(userId);
    if (player.case) throw new Error("APPROVAL_CASE_ACTIVE");
    player.case = {
      id: caseId,
      startedAt: now.toISOString(),
      readyAt: new Date(now.getTime() + approvalCaseDurationMs(caseId, player.levels.routing)).toISOString(),
    };
  }
  ...
  collect(userId: string, now = new Date()): { forms: number; coins: number } {
    ...
    const forms = approvalCaseOutput(activeCase.id, player.levels);
    const today = getUtcDayKey(now);
    const earnedToday = player.earnedDay === today ? player.earnedToday : 0;
    const coins = Math.min(approvalCaseCoins(activeCase.id, player.levels.printer), APPROVAL_DESK_DAILY_COIN_CAP - earnedToday);
    this.economy.rewardApprovalCase(userId, `${userId}:${activeCase.startedAt}`, coins, now);
    player.forms += forms;
    player.lastContribution = { forms, at: now.toISOString() };
    this.state.totalForms += forms;
  ...
  stamp(userId: string, now = new Date()): number {
    ...
    const forms = 1 + player.levels.stamp * 2;
    player.forms += forms;
    player.lastContribution = { forms, at: now.toISOString() };
    player.stamps += 1;
