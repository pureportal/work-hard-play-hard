...
describe("WorldRuntime Falling Blocks modes and attacks", () => {
  it("keeps a round alive across a brief disconnect and finishes it after the reconnect window", () => {
    const store = new WorkspaceStore(createTestData());
    const runtime = new WorldRuntime(store);
    const events: ServerEvent[] = [];
    const firstPeer = runtime.connect("user-maya", "floor-studio", (event) => events.push(event));
    try {
      runtime.handleCommand(firstPeer, { type: "movement.set_destination", requestId: "gather", floorId: "floor-studio", x: 1_250, y: 620 });
      for (let tick = 0; tick < 500; tick += 1) runtime.runTickForTest();
      runtime.handleCommand(firstPeer, { type: "game.start", requestId: "solo", definitionId: FALLING_BLOCKS_DEFINITION_ID, objectId: "object-falling-blocks", solo: true });
      const roundId = latestRound(events)!.id;
      runtime.disconnect(firstPeer);
      runtime.runTickForTest(5_000);
      expect(store.getScores().some((score) => score.roundId === roundId)).toBe(false);
  ...
  it.each<FallingBlocksSettings>([
    ...
    try {
      ...
      runtime.handleCommand(maya, { type: "game.command", roundId: latestRound(mayaEvents)!.id, requestId: "attack", command: "drop", sequence: 1, inputSessionId: "11111111-1111-4111-8111-111111111111" });
      expect(latestRound(mayaEvents)).toEqual(latestRound(leoEvents));
      ...
      expect(latestState(restoredEvents)).toMatchObject({
        roundId: latestState(leoEvents)?.roundId,
        grid: latestState(leoEvents)?.grid,
        score: latestState(leoEvents)?.score,
      });
