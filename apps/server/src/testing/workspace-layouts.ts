...
const studioRooms: RoomTemplate[] = [
  ...
  room("room-daily", 740, 250, "Daily Room", "#ccd9e9", 8, {
    organisationUnitId: "unit-engineering",
    build: { mode: "assigned", assignedPersonIds: [], ceos: true,
      unitGrants: [{ unitId: "unit-engineering", rank: "leads", descendants: false }] },
  }),
  room("room-focus", 1200, 250, "Focus Suite", "#d9d1e8", 5, {
    organisationUnitId: "unit-design",
    access: { mode: "assigned", assignedPersonIds: ["user-priya", "user-maya"], knockable: true },
    build: { mode: "assigned", assignedPersonIds: ["user-priya"] },
  }),
  room("room-product", 400, 650, "Product Studio", "#cbded4", 12, {
    organisationUnitId: "unit-engineering",
    build: { mode: "assigned", assignedPersonIds: [], ceos: true, unitGrants: [
      { unitId: "unit-engineering", rank: "members", descendants: true },
      { unitId: "unit-product", rank: "members", descendants: true },
    ] },
  }),
...
const rooftopRooms: RoomTemplate[] = [
  ...
  room("room-workshop", 900, 250, "Workshop", "#e7d6ba", 10, {
    organisationUnitId: "unit-platform",
    build: { mode: "assigned", assignedPersonIds: [],
      unitGrants: [{ unitId: "unit-platform", rank: "leads", descendants: true }] },
  }),
  room("room-quiet", 800, 560, "Quiet Corner", "#d0d8e8", 4, {
    organisationUnitId: "unit-research",
    access: { mode: "assigned", assignedPersonIds: ["user-aisha", "user-noah"], knockable: true },
    build: { mode: "assigned", assignedPersonIds: ["user-aisha"] },
  }),
...
] satisfies TestLayoutDefinition[];

interface TestLayoutDefinition {
  floorId: string;
  walls: Wall[];
...
export function createTestLayouts(floors: Floor[], objects: WorldObject[]): FloorLayout[] {
...
function room(
  ...
  color: string,
  capacity: number,
  permissions: Partial<Pick<RoomTemplate, "access" | "build" | "organisationUnitId">> = {},
): RoomTemplate {
  return {
    id,
    anchor: { x, y },
