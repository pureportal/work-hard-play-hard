...
import { createTestOrganisation } from "./workspace-organisation.js";
...
const { floors, layouts } = createTestWorld(office.id);
...
function createMessages(now: Date): ChatMessage[] {
  return [
    ...
    { id: "message-team-6", conversationId: "conversation-team", userId: "user-theo", body: "I will run the entry checks from there.", createdAt: shiftedIso(now, -112), sequence: 5 },
...
export function createTestData(currentUserId = "user-maya", now = new Date()): BootstrapData {
  ...
  return structuredClone({
    ...
    gameSettings: {
      ...
      roomBuild: { mode: "none", assignedPersonIds: [] },
    },
    kidnapping: {
