...
export function readSpotifyConfig(environment = process.env): SpotifyConfig | undefined {
  ...
  if (!clientId && !redirectUri) {
    if (tokenKey) readSpotifyEncryptionKey(tokenKey);
    return undefined;
  }
  ...
  return { clientId, redirectUri, encryptionKey: readSpotifyEncryptionKey(tokenKey) };
}

export function readSpotifyEncryptionKey(tokenKey: string): Buffer {
