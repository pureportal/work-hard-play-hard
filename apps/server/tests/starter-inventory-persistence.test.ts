...
describe("starter inventory PostgreSQL persistence", () => {
  it("saves the grant once, restores its receipts, and preserves zero resale value without backfilling existing players", async () => {
    ...
    try {
      ...
      expect(inventory.map((asset) => asset.assetId)).toEqual(["table-cafe", "decor-laptop"]);
      const saved = { players: [], store: store.exportMutableState() };
      await repository.save(saved);
      await repository.save(saved);
      expect(await orm.em.fork().count(OwnedAssetEntity, { userId })).toBe(2);
      ...
      expect(restored.disposeAsset(userId, inventory[0]!.id, "asset_sale", "sell-starter").transaction.amount).toBe(0);
      expect(restored.disposeAsset(userId, inventory[1]!.id, "asset_donation", "donate-starter", "workspace").transaction.amount).toBe(0);
      ...
      expect(finalStore.getPlayerEconomy(userId)).toMatchObject({ coinBalance: 250, inventory: [] });
      expect(finalStore.getPublicEconomy().inventory).toEqual([
        { id: inventory[1]!.id, assetId: "decor-laptop", fundId: "workspace", paid: 0 },
      ]);
      expect(finalStore.disposeAsset(userId, inventory[0]!.id, "asset_sale", "sell-starter").replayed).toBe(true);
      expect(await orm.em.fork().count(OwnedAssetEntity, { userId })).toBe(0);
    } finally {
      await orm?.close(true);
      try {
